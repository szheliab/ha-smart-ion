"""Typed Smart iON CS-8 register model.

All addresses are zero-based protocol addresses, as required by Modbus.
"""
...
class RelayOutputs(Component):
    """The eight physical relay coils (FC01/FC05)."""
...
class InputStatus(Component):
    """Input state and status flags packed in FC04 input registers."""
...
class DeviceInformation(Component):
    ...
    def serial_number(self) -> str | None:
        """Return the documented five-word serial number without guessing its format."""
        words = (
            self.serial_word_1,
            self.serial_word_2,
            self.serial_word_3,
...
class SmartIonCS8:
    """Smart iON CS-8 device composed from typed Modbus components."""
    ...
    def __init__(self, unit: ModbusUnit) -> None:
        ...
        self._fast = ComponentGroup(unit, (self.relay_outputs, self.input_status))
    ...
    async def async_update_fast(self) -> None:
        """Update relay and input state suitable for one-second polling."""
        await self._fast.async_update()

    async def async_update_settings(self) -> None:
        """Update configuration and device identification."""
