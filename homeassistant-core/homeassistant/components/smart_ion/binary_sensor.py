...
DESCRIPTIONS = (
    *tuple(
        SmartIonBinaryDescription(
            key=f"input_{index}", name=f"Input {index}", field=f"input_{index}"
        )
        for index in range(1, 9)
    ),
    *tuple(
        SmartIonBinaryDescription(
            key=f"input_status_{index}",
            name=f"Input {index} status",
            field=f"input_status_{index}",
    ...
    SmartIonBinaryDescription(
        key="auxiliary_input", name="Auxiliary input", field="auxiliary_input"
    ),
    SmartIonBinaryDescription(
        key="auxiliary_input_status",
        name="Auxiliary input status",
        field="auxiliary_input_status",
        entity_category=EntityCategory.DIAGNOSTIC,
        entity_registry_enabled_default=False,
    ),
)
...
class SmartIonBinarySensor(SmartIonEntity, BinarySensorEntity):
    ...
    def __init__(
        self, coordinator: SmartIonCoordinator, description: SmartIonBinaryDescription
    ) -> None:
